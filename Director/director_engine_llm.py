"""Standalone AnyLLM helpers for Plan-A local engines."""
...
def llm_ready(*, model: str, api_key: str) -> bool:
    """Return whether local AnyLLM runtime appears usable."""
...
def image_path_to_data_url(path: str | Path) -> str:
    """Encode one local image file as a data URL."""
...
def _close_client(llm: Any) -> None:
    """Close sync and async provider sessions when possible."""
...
def completion(
    ...
    **kwargs: Any,
    ...
    """Standalone AnyLLM completion wrapper with explicit client cleanup."""
    ...
    if AnyLLM is not None and LLMProvider is not None:
        ...
        if provider_key == LLMProvider.from_string("gemini") and api_base:
            ...
            if request_timeout_seconds and "timeout" not in http_options:
                http_options["timeout"] = int(max(1.0, request_timeout_seconds) * 1000)
    ...
    client = OpenAI(api_key=api_key, base_url=_openai_base_url(api_base), timeout=request_timeout_seconds or 120.0)
    return client.chat.completions.create(model=model, messages=messages, **kwargs)


def _extract_response_text(response: Any) -> str:
    """Extract one best-effort text body from an AnyLLM response."""
...
def _extract_first_json_object(text: str) -> str:
    """Extract the first decodable JSON object or array substring."""
...
def _parse_json_candidates(text: str) -> tuple[Optional[dict[str, Any]], Optional[str]]:
    """Try multiple tolerant JSON parsing strategies before giving up."""
...
def _repair_json_response(
    *,
    ...
    """Ask the model to convert its own reply into one strict JSON object."""
